...
func validateAnnotationPlacement(
	...
	for {
		...
		case xml.StartElement:
			...
			if len(stack) > 0 {
				...
				if !opaque {
					...
					case xml.Name{Space: Namespace, Local: "attribute"}:
						switch parent.name {
						case xml.Name{Space: Namespace, Local: "schema"},
							xml.Name{Space: Namespace, Local: "redefine"}:
					...
					case xml.Name{Space: Namespace, Local: "annotation"}:
						if parent.name != (xml.Name{Space: Namespace, Local: "schema"}) &&
							parent.elements > 0 {
							return located(
								decoder,
								systemID,
					...
					case xml.Name{Space: Namespace, Local: "enumeration"}:
						if parent.notationBase {
							for _, attribute := range value.Attr {
								if attribute.Name.Space == "" {
									switch attribute.Name.Local {
									case "value":
			...
			if !opaque {
				switch value.Name {
				case xml.Name{Space: Namespace, Local: "notation"}:
					name := ""
					identifier := false
					for _, attribute := range value.Attr {
						if attribute.Name.Space == "" {
				...
				case xml.Name{Space: Namespace, Local: "restriction"}:
					for _, attribute := range value.Attr {
						if attribute.Name.Space == "" {
							switch attribute.Name.Local {
							case "base":
	...
	for {
		...
		case xml.StartElement:
			if value.Name == (xml.Name{Space: Namespace, Local: "annotation"}) {
				...
				return nil, located(decoder, systemID, skipUnsupportedElement(decoder, value))
			}
		case xml.EndElement:
	...
	for {
		...
		case xml.StartElement:
			if value.Name == (xml.Name{Space: Namespace, Local: "annotation"}) {
				...
				return Notation{}, skipUnsupportedElement(decoder, value)
			}
		case xml.EndElement:
	...
	for {
		...
		case xml.StartElement:
			if value.Name.Space == Namespace {
				...
				default:
					return skipUnsupportedElement(decoder, value)
				}
			} else {
				return skipUnsupportedElement(decoder, value)
			}
	...
	for {
		...
		case xml.StartElement:
			if value.Name == (xml.Name{Space: Namespace, Local: "annotation"}) {
				...
				default:
					return IdentityConstraint{}, skipUnsupportedElement(decoder, value)
				}
			} else {
				return IdentityConstraint{}, skipUnsupportedElement(decoder, value)
			}
		...
		case xml.StartElement:
			if value.Name.Space == Namespace {
				...
				case "union":
					...
					simpleType.VarietyAnnotation = annotation
				default:
					return SimpleType{}, skipUnsupportedElement(decoder, value)
				}
			} else {
				return SimpleType{}, skipUnsupportedElement(decoder, value)
			}
			...
			if value.Name.Space == Namespace {
				...
				default:
					if compositor, ok := parseCompositor(value.Name.Local); ok {
						...
						return ComplexType{}, skipUnsupportedElement(decoder, value)
					}
				}
			} else {
				return ComplexType{}, skipUnsupportedElement(decoder, value)
...
func parseDerivationBody(
	...
	for {
		...
		case xml.StartElement:
			if value.Name.Space == Namespace {
				simpleRestriction := complexType.SimpleContent &&
					complexType.Derivation == DerivationRestriction
				if simpleRestriction && value.Name.Local == "simpleType" {
				...
				} else if _, facet := parseFacetKind(value.Name.Local); simpleRestriction && facet {
					if attributesStarted || complexType.AttributeWildcard != nil {
			...
			if value.Name == (xml.Name{Space: Namespace, Local: "annotation"}) {
				...
				default:
					if childCompositor, ok := parseCompositor(value.Name.Local); ok {
						...
						return ModelGroup{}, skipUnsupportedElement(decoder, value)
					}
				}
			} else {
				return ModelGroup{}, skipUnsupportedElement(decoder, value)
	...
	for {
		...
		case xml.StartElement:
			if value.Name.Space == Namespace {
				if value.Name.Local == "annotation" {
				...
				} else if compositor, ok := parseCompositor(value.Name.Local); ok {
					...
					return ModelGroupDefinition{}, skipUnsupportedElement(decoder, value)
				}
			} else {
				return ModelGroupDefinition{}, skipUnsupportedElement(decoder, value)
			}
...
func parseQName(value string, namespaces map[string]string) (QName, error) {
	...
	prefix := ""
	local := value
	if parsedPrefix, parsedLocal, found := strings.Cut(value, ":"); found {
		if parsedPrefix == "" || parsedLocal == "" || strings.Contains(parsedLocal, ":") {
			return QName{}, fmt.Errorf("xsd: invalid QName %q", value)
		}
		prefix = parsedPrefix
		local = parsedLocal
...
func parseSchemaReference(
	...
	switch kind {
	case ReferenceInclude, ReferenceRedefine:
	...
	for {
		...
		case xml.StartElement:
			if value.Name == (xml.Name{Space: Namespace, Local: "annotation"}) {
				...
				return nil, skipUnsupportedElement(decoder, value)
			}
		case xml.EndElement:
			if value.Name == start.Name {
